...
Contribuidores y créditos recientes del ciclo v0.1.13:

- @santhreal — una limpieza de 17 PR de corrección y fugas de recursos: el throttle HTTP ahora purga los buckets obsoletos de forma consciente del intervalo, de modo que el espaciado de rate-limit se mantiene (#1047), el rate limiter deja de crecer sin límite por IPs de cliente únicas (#1039), el event bus notifica y elimina a los subscribers al hacer clear (#1046), `_json_loads` queda protegido frente a JSON corrupto en columnas de la base de datos (#1045), volumen y close con NaN/inf ya no hacen fallar a `format_grounding_block` (#1043), el `inf` producido por `pct_change` con equity cero se sustituye (#1041), Shoonya tolera numéricos en cadena vacía (#1038), el drawdown se calcula desde el pico absoluto en equity negativo (#958) y las cuentas cross vacías e insolventes se marcan como liquidadas (#959), además de anclas exactas del índice de memoria (#956/#957) y un lote de parsing de códigos A-share / trade-journal (#919, #926–#935)
- @shadowinlife — se repara el enrutamiento de fallback de datos de mercado de HK con una nueva fuente Tencent HK (#1000), los backtests multi-mercado de Shadow se dividen por moneda de settlement (#997), los tickers estadounidenses sin prefijo se enrutan a `us_equity` (#994), los argumentos list/dict de MCP toleran clientes que envían JSON como string (#993), los motores de señales generados pasan el validador del sandbox de backtest (#992), y se corrige el dry-run del GC de memoria (#973)
- @honginp — el ciclo de vida completo del perpetuo USD-M: ejecución de funding y liquidación (#903), modelado de liquidación de margen aislado y cruzado (#889), persistencia ordenada de evidencia de fidelidad (#936), rebalanceo atómico de posiciones (#951), y rebalanceo histórico estricto de posiciones que preserva collateral, funding, fees y evidencia de fills (#1019)
- @QCYTSN — el ciclo de vida del shell de Electron de escritorio (#923), empaquetado para Windows con un runtime embebido fijado por checksum y manejo de credenciales con `safeStorage` (#1015), y descubrimiento de modelos en Settings + UX de identidad de runtime (#924)
- @Robin1987China — las herramientas `technical_indicators` (#921) y `sentiment` (#939), entradas de anualización para 15 fuentes de datos con una puerta de cobertura en CI (#891), lectura de inputs de indicadores sin muestreo (#1005), y el agregador de informe final `quant_strategy_desk` (#1048)
- @Shizoqua — caché de specs de MCP indexada por la identidad del servidor remoto (#1049), ranking de memoria FTS5 con decaimiento de importancia (#1032), detección en swarm de fallos de herramientas `ok`/`success:false` (#1028), y resampling `4h` en minúsculas en el loader de yfinance (#1013)
- @ngoanpv — investigación programada con conciencia de zona horaria (#954, cierra #953), un registro malformado ya no rompe todo el store (#1003), creación de intervalos sin una zona resoluble (#1004), y validación del job id en la ruta de borrado (#980)
- @he-yufeng — artefactos de risk x-ray en runs de portfolio (#900), artefactos de Portfolio Studio expuestos en run detail (#966), crypto de yfinance enrutado al motor de crypto (#970), y entradas de memoria enrutadas por jerarquía que conservan su extensión `.md` (#984)
- @wiliao — mercados canadienses de extremo a extremo: datos de mercado TSX/TSXV (#1024) y soporte de símbolos mediante fail-fast en la búsqueda y grounding con CSV (#1037)
- @MuggleJinx — historial de sesiones y runs almacenado bajo la raíz de runtime del usuario (#925, cierra #904) y carga de `~/.vibe-trading/.env` en la ruta de comandos del connector (#902, cierra #901)
- @cgycorey — las herramientas de Alpha Zoo llegan a MCP (#979) y se aceptan eventos de stream de Responses con forma de mapping (#1034)
- @ofeksh-tr — el connector de la API pública de eToro con perfiles demo/real separados por ruta (#989) — conectores de broker 12 → **13**
- @su322 — los ids de mensaje de origen se propagan al envío de IM para que las respuestas de QQ se mantengan pasivas (#1008), y el principio de output que detiene al agente una vez que la evidencia es suficiente (#1010)
- @zwrong — el comando de autoactualización `vibe-trading update` (#1020)
- @yrk111222 — ModelScope como proveedor compatible con OpenAI (#1011)
- @x-lambda — opt-out de proxy del agente que cubre clientes síncronos y asíncronos (#995)
- @dineeshd — superficies de capacidades de la API autenticadas bajo CSP forzado (#961)
- @hhj123123 — la raíz canónica de runs se preserva tras el aislamiento de HOME (#1012)
- @00EVA — el dropdown de modelos en Settings se autoajusta en tamaño para que los slugs largos de provider/modelo sigan siendo legibles (#1006)
- @darkknight4563 — `bar_returns` se mantiene bit a bit idéntico para series positivas con gaps (#895)
- @sambazhu — configuración de mootdx pre-sembrada en el home del sandbox (#982)
- @yagnikpipaliya — autorreparación para modelos de Claude que deprecan el campo `temperature` (#890, cierra #856)
- @JungHoonGhae — el motor de mercado de renta variable coreana (KRX) (#693)
- @shugaoye — el puente con OpenBB Workspace (#817)
- @TSENGCHIENFENG — la herramienta de solo lectura de snapshot de Taiwán (#848)
- @ddy4633, @tyj147454413-cmd, @y85998607, @klmtseng, @ebujinovch, @g0rdonL, @AmirF194, @Echoandelementwebsites, @dvirarad, @1anter — correcciones de correlation, loader, vn.py-export, PIT-fundamentals y alpha-bench trasladadas a este ciclo
- @warren618 / Haozhe Wu — la capa de finance-math y el motor de valuación, la columna vertebral de entity + cash-flow, los run manifests y el libro de auditoría encadenado por hash, las cuatro herramientas de datos de solo lectura, los comandos institucionales y las lentes de inversor, la reparación del grounding-gate, la herramienta analítica de payoff de opciones (#946), la recuperación de Codex OAuth (#1014), la corrección de la raíz de runtime de Shadow (#1017), integración de releases y triage de PRs/issues abiertos
