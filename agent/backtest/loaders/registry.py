...
# Every module whose import registers a loader. A named tuple rather than a
# local inside ``_ensure_registered()`` because tests swap ``LOADER_REGISTRY``
# for a temporary dict and stub out ``importlib.import_module`` to keep that
# pass fake — and a stub that *guesses* loader modules by path prefix
# (``backtest.loaders.*``) lets a loader stored elsewhere fall through to the
# real import. Its ``@register`` then lands in the temporary dict, which is
# thrown away, while ``sys.modules`` keeps the module: the process registry
# loses that loader for good. ``backtest.warehouse.loader`` is exactly such a
# loader, so the interception list has to be this list, read by name.
...
# Keep in sync with ``_LOADER_MODULES`` above — the regression test
