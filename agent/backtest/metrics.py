...
_TRADING_DAYS = {
    ...
    # warehouse is the local store, not an independent feed: it serves the bars a
    # source already delivered (today: Tushare, A-shares only, since the write
    # gate only fills markets whose canonical units are declared). Annualisation
    # therefore copies its upstream market rather than inventing a session.
    "warehouse": 252,
